  ...
  return (
    <DropdownMenu modal={false}>
      ...
      <DropdownMenuItem
          onClick={() => {
            if (email.summary) {
              setCurrentRow({ ...email, viewMode: 'summary' }) // New view-only mode
              setOpen('update') // open drawer in view mode
            } else {
              setCurrentRow({ ...email, summarize: 'summarize' }) // trigger summarize mode
              setOpen('update')
            }
          }}
        >
